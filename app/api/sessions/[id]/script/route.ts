...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/sessions/${sessionId}/script`, {
      method: 'GET',
      headers,
    });
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorData = { error: errorText || 'Failed to get script' };
      }
      return Response.json(
        errorData,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorData = { error: errorText || 'Failed to update script' };
      }
      return Response.json(
        errorData,
        { status: response.status }
